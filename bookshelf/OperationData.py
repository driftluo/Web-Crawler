...
class OperationData:
    ...
    def insert(self, dic):
        ...
        for v in dic.values():
            try:
                self.cur.execute(
                    'insert into bookinformation (bookname, latestchapter, url, bookmark) value(%s,%s,%s,%s)', tuple(v))
                print('insert %s' % v[0])
            except:
                print(sys.exc_info())
            finally:
    ...
    def updatenewchapter(self, dic):
        ...
        for v in dic.values():
            try:
                self.cur.execute(
                    'update bookinformation set latestchapter=%s, updatetime=%s where bookname=%s', tuple(v))
    ...
    def updatebookmark(self, dic):
        ...
        for v in dic.values():
            try:
                self.cur.execute(
                    'update bookinformation set bookmark=%s where bookname=%s', tuple(v))
    ...
    def delebook(self, dic):
        ...
        for v in dic.values():
            try:
                self.cur.execute(
                    'delete from bookinformation where bookname=%s', tuple(v))
    ...
    def updateurl(self, dic):
        ...
        for v in dic.values():
            try:
                self.cur.execute(
                    'update bookinformation set url=%s where bookname=%s', tuple(v))
