...
def displaydata():
    ...
    try:
        conn = pymysql.connect(host='localhost', user='root',
                               passwd='system', db='example', port=3306, charset='utf8')
        cur = conn.cursor()
        cur.execute(
            'select bookname, latestchapter, bookmark, updatetime, url from bookinformation where dr=0')
        datas = cur.fetchall()
    except:
        print(sys.exc_info())
    finally:
...
def updateurl():
    ...
    cur = conn.cursor()
    cur.execute(
        'select bookname from bookinformation where dr=0 and url is null')
    datas = cur.fetchall()
    cur.close()
    conn.close()
...
def updatenewchapter():
    ...
    num = cur.execute(
        'select url, bookname from bookinformation where dr=0 and url is not null')
    if num < num1:
        updateurl()
    datas = cur.fetchall()
    cur.close()
